...
- `FIELD`. Позволяет применять целевую аннотацию над полем класса или константой (значением) енама. Также сюда 
  включены компоненты record'ов - в целом, их почти всегда рассматривают как класс, но есть синтаксические нюансы, 
...
- `SOURCE`. Аннотация будет стерта при компиляции и не попадет в `.class`-файл. Основное назначение данной политики -
  ...
  аннотации будут заменены сгенерированными элементами класса. Непосредственно Lombok будем изучать подробнее в 
...
коде приложения такие ситуации тоже можно представить, особенно при разработке сквозной функциональности. Дальше 
...
легко забыть о конфигурации, определенной лишь единожды, где-то в глубине цепочки предков.
