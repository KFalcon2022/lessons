...
рассматриваться как компонент Spring-приложения, иными словами - объект (или объекты) данного класса должен быть создан
как бин.
...
мы будем разбираться позже, при знакомстве со Spring Data. При использовании лишь Spring Context данная 
...
ничего не значат. Чтобы вся магия заработала, необходимо каким-то образом запустить процесс инициализации
Spring-приложения, который обеспечит всю цепочку действий:

- Анализ нужных пакетов в поисках компонентов;
- Загрузка классов, фильтрация и идентификация тех, которые отмечены как компоненты;
...
случае к концу знакомства со Spring Core. Но сейчас интерфейс примечателен тем, что он является для нас 
...
Такой подход подкупает лаконичностью, особенно в проектах без библиотек кодогенерации, вроде Lombok. Особенно данный 
...
Лень же диктует искать простые пути. И внедрение в поле - это оно:
...
- Непрозрачность зависимостей. Этот пункт особенно актуален для иерархий. При отсутствии конструктора фактически 
  ...
  дублирующим зависимостям, в отдельных случаях - к циклическим зависимостям (им позже будет посвящена целая статья).
...
- Актуален тот же минус, что и для методов - есть риски упустить конфигурацию какой-то из зависимостей, что 
  останется незамеченным на старте приложения.
...
внедрить зависимость в `@Bean`-метод, это тоже станет фатальной ошибкой. Однако, с точки зрения связности кода данный 
...
Применение актуально как для всех случаев использования `@Autowired`, так и для `@Bean`-методов.
...
более одного бина подходящего типа и нужен конкретный. Если бин нужного типа всего один - он будет внедрен и без 
...
нашего приложения. И лишь инверсия управления в таких ситуациях позволит легко связать эти объекты между 
