...
С базовым синтаксисом фильтров и способом их описания в `web.xml` мы уже встречались в предыдущих статьях. Также
...
2. Логирование. В случае с фильтрами - это, скорее всего, будут разного рода общие логи, вроде пришедшего запроса и
   ...
   система обрабатывает множество запросов одновременно - это может сказаться на производительности. Поэтому подобные
   логи обычно прописывают с низким уровнем логирования - `debug` или `trace` - и включают данный уровень лишь при
...
себе как описание самого паттерна, так и раскрывает некоторые детали его реализации в фильтрах Servlet API.
...
Но важно то, что после получения целевого ресурса (страницы, мультимедиа, результата работы сервлета) процесс 
...
Подобное поведение не является типовым для цепочки обязанностей в классической реализации, но широко применяется для 
...
protected void doFilter(HttpServletRequest req, HttpServletResponse res, FilterChain chain) 
    ...
    // но данный подход нагляднее для демонстрации
...
protected void doFilter(HttpServletRequest req, HttpServletResponse res, FilterChain chain)
    ...
    // Сериализуем объект в JSON, представленный в виде массива байт
