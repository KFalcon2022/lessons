...
Конкретно сегодня мы практически не затронем реальное применение рассматриваемых инструментов и концепций, но пункты
...
> приложения (того же браузера) платформой можно считать операционную систему. Платформой для операционной системы
...
> обработать конкретный запрос. Это позволяет уйти от ряда неудобств, которые вызваны использованием сервлетов. И
...
логика будет срабатывать до и/или после того, как запрос будет обработан сервлетом. Таким образом фильтр может
...
Также стоит отметить, что описанные пункты могут запускаться не только при старте контейнера, но и при других
